...
\subsubsection{Rotational symmetries in the binary phase modulation}
...
%@TODO: the computation of CGH for binary SLM also need a binary quantisation step. Although few multi-level phase modulators are available, their bit depth is limited (e.g. 4-bit 8-bit), so the quantisation step is still inevitable.
...
This section reviews the existing methods for calculating phase-only holograms. The phase-only hologram is labelled as $H$, so that it differs from the previous notation of $A$ for complex-valued hologram apertures. And the propagation function is unified as $\mathcal{P}$, which can be either the Fraunhofer propagation equation in \cref{eq:fresnel-diffraction} or the Fresnel propagation equation in \cref{eq:fraunhofer-diffraction} depending on the distance of the target field.


\subsection{Naive Algorithm}\label{sec:Naive algorithm}
The simplest method to get a phase hologram is by directly extracting the phase of the reverse propagation from the target field, and discarding the amplitude component (e.g. for Fraunhofer propagation, the phase hologram will simply be the phase of the inverse Fourier transform $\mathcal{F} ^{-1}$ of the target image). This method is named as `Naive' algorithm here for its simplicity. The pseudocode of Naive algorithm is shown in \cref{alg:Naive algorithm} below:
\begin{algorithm}[H]
  ...
  \end{algorithmic}
\end{algorithm}
where $j = \sqrt{-1} $. The Naive algorithm (as described in \cref{alg:Naive algorithm}) was then implemented in MATLAB and the output results are shown in \cref{fig:Naive algorithm output} below:
